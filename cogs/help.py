...
class Help(commands.Cog):
  ...
  async def help(self,ctx,cat=None):
    ...
    if not cat:
      embed=discord.Embed(title="Help menu for Mootbot",description=f"Prefix: `{prefix}`\nCommands: \nmute, unmute, softban, ban, kick, warn, purge, setup (raid protection setup), warns, log, logtoggle (turns logs on or off), avatar, userinfo, say, rr",color=colour,timestamp=datetime.utcnow())
